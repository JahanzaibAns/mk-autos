...
@php
    ...
    if (isset($brand)) {
        ...
        $pageName = $brand->name ?? 'Car';

    } elseif (isset($category)) {
        $metaTitle = $category->meta_title ?? '';
        $metaDescription = $category->meta_description ?? '';
        $pageName = $category->name ?? 'Car';

    } elseif (isset($bodyType)) {
        $pageName = $bodyType->body_type ?? 'Car';
    ...
    // Fallback if meta not set in DB
    if (empty($metaTitle)) {
        $metaTitle = 'Rent ' . $pageName . ' Cars in Dubai | MK Luxury Car Rental';
    }

    if (empty($metaDescription)) {
        $metaDescription = 'Rent ' . $pageName . ' cars in Dubai at best price. Daily, weekly & monthly rental available at MK Luxury Car Rental.';
    }
@endphp
...
<section class="our_fleet_sec pt-150">
    <div class="container">
<h2 class="secHeading">{{ $pageName }} Car Rental Dubai</h2>
