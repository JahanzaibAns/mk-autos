...
@php
    ...
    if (isset($category)) {
        ...
        $fullCategoryName = $category->category ?? 'Car';
        ...
        $metaDescription = $firstCategory->meta_description ?? null;
        $categoryDescription = $firstCategory->description ?? '';
    }

    // Fallback meta if DB fields are empty
    if (empty($metaTitle)) {
        $metaTitle = $fullCategoryName . ' Cars for Rent in Dubai | MK Luxury Car Rental';
    }

    if (empty($metaDescription)) {
        $metaDescription = 'Rent ' . strtolower($fullCategoryName) . ' cars in Dubai with MK Luxury Car Rental. Enjoy comfort, reliability, and affordable rides for daily travel, city drives, and short trips.';
    }

    // First word only (if you still need it somewhere)
    $categoryName = ucfirst(explode(' ', trim($fullCategoryName))[0]);
@endphp
...
@section('content')
<style>
    .description_Car_Row_div ul, .description_Car_Row_div ol {
        margin: 0 0 16px 20px;
        padding: 0;
    }
    .description_Car_Row_div li {
        padding: 0px !important;
        list-style: disc;
    }
    .description_Car_Row_div h1, .description_Car_Row_div h2, .description_Car_Row_div h3, .description_Car_Row_div h4 {
        margin-top: 20px;
        margin-bottom: 10px;
        font-weight: 600;
    }
    /* Anchor styles */
.description_Car_Row_div a {
    color: #000000;
    text-decoration: none;
    transition: color 0.3s ease;
}

.description_Car_Row_div a:hover {
    color: #BB9A21;
...
<section class="our_fleet_sec pt-150">
    <div class="container">
        <ul class="breadcrumb">
                ...
                <li class="breadcrumb-item">Car Types / {{ $fullCategoryName }}
                </li>
            </ul>
        <h2 class="secHeading">{{ $fullCategoryName }} for Rent in Dubai</h2>
        <div class="col-md-10">@if($categoryName === 'Economy')
Travel on a budget? MK Luxury Car Rental delivers affordable economy options. Choose new, reliable vehicles with daily, weekly, or monthly plans at competitive rates. Economy cars provide maximum value without cutting quality.
@elseif($categoryName === 'Luxury')
Drive luxury cars across Dubai with MK Luxury Car Rental. Experience premium models at sharp rates with effortless bookings—ideal for weddings, business events, or quick getaways. Book your Dubai luxury rental now.
@elseif($categoryName === 'Sports')
MK Luxury Car Rental offers premium vehicles in Dubai from AED 600 daily. Drive Ferrari, Lamborghini, BMW, Mercedes, and more—all meticulously maintained for performance and comfort. Enjoy 24/7 support and easy bookings. Reserve your standout ride today.
@elseif($categoryName === 'Exotic')
Your arrival sets the tone silently. Choose a car that showcases your taste, confidence, and detail for meetings, family gatherings, or special occasions. The right vehicle elevates you—prepared, polished, and unforgettable beyond simple transport.
@elseif($categoryName === 'SUV')
Elevate Dubai adventures with powerful SUV rentals. These vehicles deliver rugged capability, spacious luxury, and versatility for city streets or desert dunes. Arrive ready for any journey.
@elseif($categoryName === 'Convertible')
Crave open-air luxury? MK Luxury delivers premium convertibles at smart rates. Flexible daily, weekly, or monthly plans provide stylish drop-tops with maximum thrill and zero prestige compromise.
@endif</div>
        <hr/>
        ...
        <div class="row g-3 footer_social_boxes social_boxes_category mb-5 justify-content-center">
            ...
            @if($categoryDescription)
                <div class="category-description mb-4 description_Car_Row_div">
                    {!! $categoryDescription !!}
                </div>
            @endif
